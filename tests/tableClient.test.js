...
describe('TableClient', () => {
  ...
  describe('getRecords', () => {
    ...
    test('should call apiClient.get with order_by parameter', async () => {
      // Mock the API response
      mockApiClient.get.mockResolvedValueOnce({
        data: [
          { id: 2, name: 'Product 2' },
      ...
      // Call the method with order_by parameter
      const result = await tableClient.getRecords('products', {
        order_by: 'name DESC'
      });

      // Check that apiClient.get was called correctly with order_by parameter
      expect(mockApiClient.get).toHaveBeenCalledWith('/api/tables/products', {
        page: 1,
        pageSize: 10,
        order_by: 'name DESC'
    ...
    test('should call apiClient.get with order_by, pagination, and filter parameters', async () => {
      // Mock the API response
      mockApiClient.get.mockResolvedValueOnce({
        data: [
      ...
      // Call the method with order_by, pagination, and filter parameters
      const result = await tableClient.getRecords('products', {
        page: 1,
        pageSize: 5,
        order_by: 'price DESC',
        filter: {
          category: 'electronics',
          order_by: 'name ASC' // This should not override the order_by parameter
        }
      });

      // Check that apiClient.get was called correctly with all parameters
      expect(mockApiClient.get).toHaveBeenCalledWith('/api/tables/products', {
        page: 1,
        pageSize: 5,
        order_by: 'price DESC',
